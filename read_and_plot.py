...
class Data_Reader:
    ...
    def __init__(self, filename):
        ...
        print(self.solution)
        self.lookup_df = pd.read_excel(self.xls, sheet_name='lookup dict', index_col=0, header=0)

        self.meta_lookup = self.lookup_df.to_dict('dict')[0]  # has the form of dict {0 : dictionary I want}

    def set_metabolites_to_plot(self, list_of_metabolites):
        self.metabolites_to_plot = list_of_metabolites

    def overlapping_plot_results_of_interest(self, existing_ax, label_modifier, linestyle):  # , existing_ax, label_modifer):
        ...
        if existing_ax is None:
            fig, ax = plt.subplots()
            ax.plot(self.time, values_of_interest, label=[label_modifier + meta for meta in self.metabolites_to_plot], linewidth=3)
            # plt.ylim([0, 3])
            # plt.legend()
            return ax
        else:
            ...
            existing_ax.plot(self.time, values_of_interest, label=new_labels, linestyle=linestyle, linewidth=3)
            return existing_ax
...
def plot_difference(dr_healthy, dr_disease):
    ...
    for key in healthy_lookup.keys():
        ...
        difference = [disease_vals[i] - healthy_vals[i] for i in range(0, len(healthy_vals))]
    ...
    plt.plot(dr_healthy.time, [0]*len(dr_healthy.time), linestyle='dashed', color='black')
    plt.legend()
    plt.ylabel('differenc in metabolite level')
    plt.xlabel('time in hours')
    plt.title('Disease State Simulation - Healthy State Simulation')
    plt.ylim([-0.75, 0.75])
    plt.show()
...
if __name__ == '__main__':
    ...
    metas_to_plot = ['glucosylceramide_0', 'gba_0', 'a_syn_0', 'a_syn_1', 'mis_a_syn_0', 'mis_a_syn_1', 'a_syn_proto_0',
                     'mutant_lrrk2_0', 'clearance_0']
    ...
    sig_different_traj_lrrk2 = determine_different_trajectories(dr_low_mutant_lrrk2, dr_high_mutant_lrrk2)
    ...
    plot_difference(dr_healthy=dr_high_gba, dr_disease=dr_low_gba)
    plot_difference(dr_healthy=dr_low_mutant_lrrk2, dr_disease=dr_high_mutant_lrrk2)